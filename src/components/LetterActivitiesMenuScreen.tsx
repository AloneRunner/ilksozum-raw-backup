...
const LetterActivitiesMenuScreen: React.FC<LetterActivitiesMenuScreenProps> = ({ onSelectActivity, onBack, activityStats, theme, enabledActivities }) => {
  ...
  const activities = [
    { id: ActivityType.FindTheLetterInGrid, icon: GridIcon, title: "Harf Tablosu", subtitle: "Karışık harflerden isteneni bul.", color: 'amber' as const, hasSubCategories: true },
    { id: ActivityType.SoundPresence, icon: SpeakerIcon, title: "Seste Harf Var mı?", subtitle: "Görselin adında o ses var mı?", color: 'cyan' as const, hasSubCategories: true },
    { id: ActivityType.FindTheLetter, icon: SearchIcon, title: "Harf Bulma Oyunu", subtitle: "Kelimenin içindeki harfleri bul.", color: 'indigo' as const, hasSubCategories: true },
    { id: ActivityType.FindTheSoundInImage, icon: ImageIcon, title: "Görselde Sesi Bul", subtitle: "İstenen sesi içeren görseli seç.", color: 'teal' as const, hasSubCategories: true },
    { id: ActivityType.Syllabification, icon: SyllableIcon, title: "Heceleme Oyunu", subtitle: "Kelimeleri hecelerine ayır.", color: 'rose' as const, hasSubCategories: true },
    { id: ActivityType.EmbeddedStory, icon: BookOpenIcon, title: "Hikaye Zamanı", subtitle: "Harfle ilgili hikayeleri dinle.", color: 'purple' as const, hasSubCategories: false },
  ];
  ...
  const titleColorClass = isThemed ? 'text-white text-shadow-soft' : 'text-sky-800';
  const subtitleColorClass = isThemed ? 'text-white/90 text-shadow-soft' : 'text-slate-600';
  const iconColorClass = isThemed ? 'text-white' : 'text-sky-700';
  ...
  return (
    <div className="flex flex-col items-center justify-start h-full max-w-lg mx-auto p-4 sm-landscape:p-2 animate-fade-in overflow-y-auto">
      ...
      <p className={`text-center mb-6 sm-landscape:mb-3 text-lg sm-landscape:text-base ${subtitleColorClass}`}>
        Bir etkinlik seç ve harfleri öğrenmeye başla!
      </p>
