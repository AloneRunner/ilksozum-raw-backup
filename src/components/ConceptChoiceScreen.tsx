...
const SPAM_GUARD_CALM_MODE_MS = 45000; // After a warning, keep slower pacing for this long
...
const ConceptChoiceScreen: React.FC<ConceptChoiceScreenProps> = ({
    ...
    isSpamGuardEnabled, spamGuardRoundThreshold, activeProfileName
}) => {
    ...
    const spamTapHistoryRef = useRef<number[]>([]);
    const spamLockTimeoutRef = useRef<number | null>(null);
    const spamCountdownIntervalRef = useRef<number | null>(null);
    const spamLockEndsAtRef = useRef<number | null>(null);
    const calmModeTimeoutRef = useRef<number | null>(null);
    ...
    const guardDisplayName = React.useMemo(() => {
        if (!activeProfileName || !activeProfileName.trim()) {
            return t('spamGuard.friendName', 'arkadaşım');
        }
        return activeProfileName.split(' ')[0];
    }, [activeProfileName, currentLang]);

    const releaseCalmMode = useCallback(() => {
        if (calmModeTimeoutRef.current) {
            window.clearTimeout(calmModeTimeoutRef.current);
            calmModeTimeoutRef.current = null;
        }
        setIsCalmModeActive(false);
    }, []);

    const activateCalmMode = useCallback(() => {
        if (calmModeTimeoutRef.current) {
            window.clearTimeout(calmModeTimeoutRef.current);
        }
        setIsCalmModeActive(true);
        calmModeTimeoutRef.current = window.setTimeout(() => {
            releaseCalmMode();
        }, SPAM_GUARD_CALM_MODE_MS);
    }, [releaseCalmMode]);

    const clearSpamTimers = useCallback(() => {
        if (spamLockTimeoutRef.current) {
            window.clearTimeout(spamLockTimeoutRef.current);
            spamLockTimeoutRef.current = null;
        }
        if (spamCountdownIntervalRef.current) {
            window.clearInterval(spamCountdownIntervalRef.current);
            spamCountdownIntervalRef.current = null;
        }
    }, []);

    const releaseSpamLock = useCallback(() => {
        clearSpamTimers();
        setIsSpamLocked(false);
        setSpamCountdown(0);
        setSpamWarningMessage('');
        spamLockEndsAtRef.current = null;
        spamTapHistoryRef.current = [];
    }, [clearSpamTimers]);

    const startSpamLock = useCallback(() => {
        clearSpamTimers();
        spamTapHistoryRef.current = [];
        const friendlyName = guardDisplayName;
        const overlayText = t('spamGuard.overlayMessage', '{name}, ekrana bakıyor musun? Dikkatin burada mı? Çok hızlı dokunuyorsun, birlikte minik bir nefes alalım.').replace('{name}', friendlyName);
        setSpamWarningMessage(overlayText);
        setIsSpamLocked(true);
        activateCalmMode();
        const lockEndsAt = Date.now() + SPAM_GUARD_LOCK_MS;
        spamLockEndsAtRef.current = lockEndsAt;
        const updateCountdown = () => {
            const msLeft = Math.max(0, (spamLockEndsAtRef.current ?? 0) - Date.now());
            setSpamCountdown(Math.max(0, Math.ceil(msLeft / 1000)));
        };
        updateCountdown();
        spamLockTimeoutRef.current = window.setTimeout(() => {
            releaseSpamLock();
        }, SPAM_GUARD_LOCK_MS);
        spamCountdownIntervalRef.current = window.setInterval(updateCountdown, 200);
        if (!settings.isMuted) {
            const speechText = t('spamGuard.speechMessage', '{name}, ekranda kalalım olur mu? Ekrana bakıyor musun, dikkatin burada mı? Birlikte sakinleşip az sonra tekrar deneyeceğiz.').replace('{name}', friendlyName);
            void speak(speechText, getSpeechLocale(currentLang));
        }
    }, [activateCalmMode, clearSpamTimers, currentLang, guardDisplayName, releaseSpamLock, settings.isMuted]);

    const registerSpamTap = useCallback(() => {
        if (!isSpamGuardEnabled) return false;
        const now = Date.now();
        const history = spamTapHistoryRef.current.filter(ts => now - ts <= SPAM_GUARD_WINDOW_MS);
        history.push(now);
        spamTapHistoryRef.current = history;
        const threshold = Math.max(3, spamGuardRoundThreshold || 0);
        if (history.length >= threshold) {
            startSpamLock();
            return true;
        }
        return false;
    }, [isSpamGuardEnabled, spamGuardRoundThreshold, startSpamLock]);
    ...
    }, [clearSpamTimers, releaseCalmMode]);

    useEffect(() => {
        releaseSpamLock();
    }, [roundData.id, releaseSpamLock]);

    useEffect(() => {
        if (!isSpamGuardEnabled) {
            releaseSpamLock();
            releaseCalmMode();
        }
    }, [isSpamGuardEnabled, releaseCalmMode, releaseSpamLock]);
    ...
    const handleCardClick = async (optionId: number, isOptionCorrect: boolean, spokenText: string) => {
        ...
        if (isSpamGuardEnabled) {
            ...
            const hitLimit = registerSpamTap();
            if (hitLimit) return;
        }
        if (isCorrect) return;
    ...
    const questionConcept = (conceptFromKey || localizedQuestion.split(' ')[0]).toLocaleLowerCase(locale).trim();
        ...
        if (isOptionCorrect) {
            ...
            const allowFastIncorrect = isFastTransitionEnabled && !isCalmModeActive;
            if (allowFastIncorrect) {
    ...
    const spamOverlay = (isSpamGuardEnabled && isSpamLocked) ? (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-slate-900/70 backdrop-blur-sm px-6 text-center">
            <div className={`max-w-sm w-full rounded-3xl shadow-2xl ${isCosmic ? 'bg-slate-900/80 border border-sky-400/40' : 'bg-white'} p-6 flex flex-col items-center gap-3`}>
                <p className="text-2xl font-bold text-rose-500">{t('spamGuard.overlayTitle', 'Nazikçe Bekleyelim')}</p>
                <p className="text-base text-slate-700">{spamWarningMessage || t('spamGuard.overlayFallback', 'Ekrana odaklanıp nefes aldıktan sonra devam edeceğiz.')}</p>
                <div className="text-sm font-semibold text-slate-500">
                    {t('spamGuard.timer', '{seconds} sn').replace('{seconds}', String(Math.max(spamCountdown, 1)))}
                </div>
            </div>
        </div>
    ) : null;
